...
## FC
```{r}
FC.ctrl = corTs(ts[,,is.ctrl])
FC.pati = corTs(ts[,,is.pati])
FC.all = corTs(ts)

# Fischer's Transform
Z.R = rmdiag(0.5*(log(1+FC.all$R) - log(1-FC.all$R)))
stats.FC.R   = ttest.nettest(Z.R, mydata$group, alpha = .05, perm = F, fdr = .05)
Z.PC = rmdiag(0.5*(log(1+FC.all$PC) - log(1-FC.all$PC)))
stats.FC.PC   = ttest.nettest(Z.PC, mydata$group, alpha = .05, perm = F, fdr = .05)
```

## Plot FC
```{r fig.height=4.6, fig.width=8, message=FALSE}
p1 = gplotMat(rmdiag(FC.ctrl$R_mean), title = "controls R", barWidth = 0.3,
              nodeLabels = mylabels, xAngle = 90, axisTextSize = 8) +
  scale_x_continuous(breaks = 0.5:br, labels = mylabels)

p2 = gplotMat(rmdiag(FC.pati$R_mean), title = "patients R", barWidth = 0.3,
              nodeLabels = mylabels, xAngle = 90, axisTextSize = 8) +
  scale_x_continuous(breaks = 0.5:br, labels = mylabels)

p3 = gplotMat(stats.FC.R$stat_uncorr, title = "t-test (uc.)", barWidth = 0.3, 
              gradient = c("blue", "white", "red"), lim = c(-5, 5),
              nodeLabels = mylabels, xAngle = 90, axisTextSize = 8) +
  scale_x_continuous(breaks = 0.5:br, labels = mylabels)

p4 = gplotMat(rmdiag(FC.ctrl$PC_mean), title = "controls PC", barWidth = 0.3,
              nodeLabels = mylabels, xAngle = 90, axisTextSize = 8) +
  scale_x_continuous(breaks = 0.5:br, labels = mylabels)

p5 = gplotMat(rmdiag(FC.pati$PC_mean), title = "patients PC", barWidth = 0.3,
              nodeLabels = mylabels, xAngle = 90, axisTextSize = 8) +
  scale_x_continuous(breaks = 0.5:br, labels = mylabels)

p6 = gplotMat(stats.FC.PC$stat_uncorr, title = "t-test (uc.)", barWidth = 0.3, 
              gradient = c("blue", "white", "red"), lim = c(-5, 5),
              nodeLabels = mylabels, xAngle = 90, axisTextSize = 8) +
  scale_x_continuous(breaks = 0.5:br, labels = mylabels)

plot_grid(p1, p2, p3, p4, p5, p6, ncol = 3, nrow = 2, rel_widths = c(1, 1))
ggsave(path = PATH_FIGS, sprintf('DGM_%s-FC.png', INFO))
```
...
# using the union to form the group network
...
# this may be hard to justify so we do not do this.
...
### Plot group networks
```{r fig.height=2.4, fig.width=5, message=FALSE}
...
no.dyn.models = t(as.matrix(apply(DE < 1, 1, sum)))
rownames(no.dyn.models) = c('no. of subjects with dynamic model')
colnames(no.dyn.models) = 1:Nn
print(no.dyn.models)
```
...
```{r}
set.seed(1980)
stats.theta.mean = ttest.nettest(thetas.mean, mydata$group, alpha = .05, fdr = .05)
stats.theta.sd   = ttest.nettest(thetas.sd, mydata$group, alpha = .05, perm = T, fdr = .05)

stats.theta.mean.dyn = ttest.nettest(thetas.mean.dyn, mydata$group, alpha = .05, fdr = .05)
stats.theta.sd.dyn   = ttest.nettest(thetas.sd.dyn, mydata$group, alpha = .05, fdr = .05, perm = T)
```
...
lim = max(abs(stats.theta.mean$stat), na.rm = T)

p2 = gplotMat(stats.theta.mean$stat_uncorr, title = "t-test (uc.)", barWidth = 0.3, 
              gradient = c("blue", "white", "red"), lim = c(-lim, lim),
              nodeLabels = mylabels, xAngle = 90, axisTextSize = 8) +
  scale_x_continuous(breaks = 0.5:br, labels = mylabels)

p3 = gplotMat(stats.theta.mean$stat_fdr, title = "t-test (fdr)", barWidth = 0.3, 
              gradient = c("blue", "white", "red"), lim = c(-lim, lim),
              nodeLabels = mylabels, xAngle = 90, axisTextSize = 8) +
  scale_x_continuous(breaks = 0.5:br, labels = mylabels)
...
lim = max(abs(stats.theta.mean.dyn$stat), na.rm = T)

p5 = gplotMat(stats.theta.mean.dyn$stat_uncorr, title = "t-test (uc.)", barWidth = 0.3, 
              gradient = c("blue", "white", "red"), lim = c(-lim, lim),
              nodeLabels = mylabels, xAngle = 90, axisTextSize = 8) +
  scale_x_continuous(breaks = 0.5:br, labels = mylabels)

p6 = gplotMat(stats.theta.mean.dyn$stat_fdr, title = "t-test (fdr)", barWidth = 0.3, 
              gradient = c("blue", "white", "red"), lim = c(-lim, lim),
              nodeLabels = mylabels, xAngle = 90, axisTextSize = 8) +
  scale_x_continuous(breaks = 0.5:br, labels = mylabels)
...
p2 = gplotMat(stats.theta.sd$stat_uncorr, title = "t-test (uc.)", barWidth = 0.3, 
              gradient = c("blue", "white", "red"), lim = c(-lim, lim),
              nodeLabels = mylabels, xAngle = 90, axisTextSize = 8) +
  scale_x_continuous(breaks = 0.5:br, labels = mylabels)

p3 = gplotMat(stats.theta.sd$stat_fdr, title = "t-test (fdr)", barWidth = 0.3, 
              gradient = c("blue", "white", "red"), lim = c(-lim, lim),
              nodeLabels = mylabels, xAngle = 90, axisTextSize = 8) +
  scale_x_continuous(breaks = 0.5:br, labels = mylabels)
...
lim = max(abs(stats.theta.sd.dyn$stat), na.rm = T)

p5 = gplotMat(stats.theta.sd.dyn$stat_uncorr, title = "t-test (uc.)", barWidth = 0.3, 
              gradient = c("blue", "white", "red"), lim = c(-lim, lim),
              nodeLabels = mylabels, xAngle = 90, axisTextSize = 8) +
  scale_x_continuous(breaks = 0.5:br, labels = mylabels)

p6 = gplotMat(stats.theta.sd.dyn$stat_fdr, title = "t-test (fdr)", barWidth = 0.3, 
              gradient = c("blue", "white", "red"), lim = c(-lim, lim),
              nodeLabels = mylabels, xAngle = 90, axisTextSize = 8) +
  scale_x_continuous(breaks = 0.5:br, labels = mylabels)
...
for (n in 1:Nn) {
  ...
  s = t.test(x ~ g, var.equal = T) # use perm test!
  tvals[n] = s$statistic
  pvals[n] = s$p.value
}
...
## Compare stationary vs dynamic models?
```{r}
x = array(NA, dim = c(2, 2))
chi.stat = array(NA, dim=c(2,Nn))
rownames(chi.stat) = c("Chi-sq", "p-val")
colnames(chi.stat) = 1:Nn

df=1
for (n in which(no.dyn.models < (N*0.75))) { # edge 9,10 are dynamic
  x[1,1] = sum(DE[n,is.ctrl] < df)
  x[1,2] = sum(DE[n,is.ctrl] >= df)
  
  x[2,1] = sum(DE[n,is.pati] < df)
  x[2,2] = sum(DE[n,is.pati] >= df)
  
  tmp = chisq.test(x)
  chi.stat[1,n] = tmp$statistic
  chi.stat[2,n] = tmp$p.value
}
print(chi.stat)
```
